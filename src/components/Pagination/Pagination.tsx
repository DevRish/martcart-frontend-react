...
const Pagination = ({ page, limit, setPage, total } : IPaginationProps) => {
    console.log((total/limit) + ((total%limit === 0) ? 0 : 1))
    return (
        <div className="pagination">
            ...
            {
                [...Array(Math.round(total/limit) + ((total%limit === 0) ? 0 : 1)).keys()].map((e) => {
                    return <div className={(page === (e+1)) ? "active" : ""} onClick={() => { setPage(e+1) }}>{e+1}</div>
                })
            }
            <div className="control" onClick={() => {
                if(page < (Math.round(total/limit) + ((total%limit === 0) ? 0 : 1))) setPage(page+1);
